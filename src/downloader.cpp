...
Downloader::Downloader(const std::string &_ip, int _port,
                       const std::string &_filename, int _file_size)
    : ip(_ip), port(_port), filename(_filename), file_size(_file_size),
      file_stream(_filename, std::ios::out | std::ios::binary), socket(0),
      buffer(WINDOW_SIZE) {
...
void Downloader::send_requests() {
  ...
  while (sent < MAX_SENT_PACKETS_BATCH &&
         checkpoint + already_done < file_size && checkpoint < WINDOW_SIZE) {
    int to_send = std::min(CHUNK_SIZE, file_size - already_done - checkpoint);

    if (chunks.find(checkpoint) == chunks.end()) {
      std::string msg = "GET " + std::to_string(already_done + checkpoint) +
                        " " + std::to_string(to_send) + "\n";
...
void Downloader::listen_receive() {
  ...
  while (true) {
    ...
    ssize_t datagram_len =
        socket.recv_from(recv_buf, sizeof(recv_buf), sender, MSG_DONTWAIT);

    if (datagram_len < 0)
      break;

    process_datagram(
        std::string_view(recv_buf, static_cast<std::size_t>(datagram_len)),
        sender);
  }
}

void Downloader::process_datagram(std::string_view packet,
                                  const sockaddr_in &sender) {
  if (packet.size() < 5)
    return;

  if (sender.sin_family != server_addr.sin_family ||
      sender.sin_port != server_addr.sin_port ||
  ...
  const std::string_view start_text =
      packet.substr(prefix.size(), separator - prefix.size());
  const std::string_view len_text =
      packet.substr(separator + 1, newline - separator - 1);
...
void Downloader::process_data(int start, int len, std::string_view payload) {
  ...
  if (current_write_pos + already_done >= file_size ||
      current_write_pos >= WINDOW_SIZE) {
    flush_file();
  }
}
...
void Downloader::flush_file() {
  ...
  const double progress = 100.0 * static_cast<double>(already_done) /
